...
async def main(args):
    ...
    try:
        ...
        print(f"create an agent: {agent.agent_id}")
        
        print("Start a server...")
        await agent.start()
        print("The server has been started")
        
        print(f"Register with the gateway : {args.gateway_address}")
        await agent.register_to_gateway(args.gateway_address)
        print("The gateway has been registered")
        
        
        await run_example_operations(agent)
        
    except Exception as e:
        print(f"An uncaught exception occured: {str(e)}")
        import traceback
        traceback.print_exc()
    finally:
        ...
        if agent is not None:
            try:
                print("Shut down the agent...")
                await agent.stop()
                print("The agent has been shut down")
            except Exception as stop_e:
                print(f"Error occurred when closing the agent: {str(stop_e)}")
