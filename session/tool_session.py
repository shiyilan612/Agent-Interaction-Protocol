...
from grpc_service.tool_service import ToolService
...
class Tool(ToolService):
        ...
        if not arguments:
            ...
            for param_name in sig.parameters:
                if param_name not in arguments:
                    ...
                    arguments[param_name] = str(param.annotation) if param.annotation != inspect.Parameter.empty else "any"
    ...
    async def run(self, gateway_address: str = None) -> None:
        ...
        if self.gateway_address:
            ...
            await self.connect_to_gateway(self.gateway_address)
            print(f"Tool {self.name} registered with gateway at {self.gateway_address}")
        
        # Wait for server to complete
        await server_task
        
        
    async def call_direct(self, arguments: Dict[str, str] = None) -> ToolResponse:
        """
        call this tool directly without gateway
        """
        if not arguments:
            arguments = {}
